...
export default function DashboardPage() {
    ...
    if (!isLoaded) {
        return (
            <main className="relative w-full h-screen overflow-hidden bg-[#020617] flex items-center justify-center">
            ...
            <main className="relative w-full h-screen overflow-hidden bg-[#020617] text-slate-100 flex items-center justify-center font-sans">
                ...
                >
                    ...
                    <div className="space-y-4">
                        {[
                            { role: "Citizen", desc: "Public Advisory & Visual Risk", icon: Users, color: "emerald" },
                            { role: "Ward Officer", desc: "Local Zone Command", icon: MapPin, color: "yellow" },
                            { role: "City Admin", desc: "Delhi Wide Optimization", icon: Activity, color: "orange" },
                            { role: "System Admin", desc: "Full Threshold Control", icon: Database, color: "red" },
                        ].map((r, i) => {
                            const Icon = r.icon;
                            return (
                                <SignInButton key={i} mode="modal" fallbackRedirectUrl="/dashboard">
                                    <button
    ...
    const clerkRole = (user.publicMetadata.role as string) || 'Citizen';
    const authState = { 
        role: clerkRole, 
        ward_id: clerkRole === 'Ward Officer' ? 'Ward H-West' : null 
    };

    // Citizen gets dedicated full experience
    if (authState.role === 'Citizen') {
        return <CitizenDashboard onLogout={() => signOut()} />;
    }

    // Ward Officer gets dedicated 2D tactical dashboard (no 3D map)
    if (authState.role === 'Ward Officer') {
        return <WardOfficerDashboard onLogout={() => signOut()} />;
    }
    ...
    return (
        <main className="relative w-full h-screen overflow-hidden bg-[#020617] text-slate-100 font-sans selection:bg-blue-500/30">
            ...
            {showSysAdmin && authState?.role === 'System Admin' && (
                ...
                />
                <UserManagementModal onClose={() => setShowSysAdmin(false)} />
                </>
            )}
