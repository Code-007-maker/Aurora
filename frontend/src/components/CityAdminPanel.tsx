...
import {
    ...
    LayoutGrid, Wallet, Wrench, AreaChart, Radio, Info, Mail
} from 'lucide-react';
...
// 6. PERSONNEL / WARD OFFICER INVITES
function PersonnelInviteTab() {
        ...
        <div className="space-y-6">
            <div className="p-4 rounded-xl bg-indigo-500/5 border border-indigo-500/20">
                <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="text-sm font-bold text-white uppercase tracking-wider">Invite Ward-Level Personnel</h4>
                </div>
                <p className="text-xs text-slate-400 mb-6 leading-relaxed">
                    Deploy new Ward Officers to the field. Invited personnel will have access to 
                    <span className="text-indigo-400 font-bold"> Local Tactical Dashboards</span> for their assigned zones.
                </p>

                <form onSubmit={handleInvite} className="space-y-4">
                    <div>
                        <label className="text-[10px] text-slate-500 uppercase tracking-widest block mb-2 font-bold">Officer Email Address</label>
                        <div className="relative">
                            ...
                            <input
                                ...
                                placeholder="ward.officer@mcd.gov.in"
                                required
                                className="w-full bg-slate-900 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-sm text-white focus:outline-none focus:border-indigo-500/50 transition-colors"
                            />
                        </div>
                    </div>
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-all shadow-[0_0_20px_rgba(79,70,229,0.3)] flex items-center justify-center gap-2"
                    >
                        {loading ? <Activity className="w-4 h-4 animate-spin" /> : <Mail className="w-4 h-4" />}
                        {loading ? 'Processing Deployment...' : 'Send Deployment Invite'}
                    </button>
                </form>
                ...
                {message && (
                    <motion.div
                        ...
                        className={`mt-4 p-3 rounded-lg text-xs font-bold border ${message.startsWith('Error') ? 'bg-red-500/10 border-red-500/30 text-red-400' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'}`}
                    >
                        {message}
                    </motion.div>
                )}
            ...
            <div className="p-4 rounded-xl bg-white/[0.02] border border-white/5 font-mono">
                <h5 className="text-[10px] text-slate-500 uppercase tracking-widest mb-2 font-black">Officer Responsibilities</h5>
                <ul className="text-[10px] text-slate-400 space-y-1">
                    <li>· Monitor Local Flood Sensors</li>
                    <li>· Reports Field Discrepancies</li>
                    <li>· Direct Pump Deployment teams</li>
                    <li>· Receive Automated Alert Stream</li>
                </ul>
            </div>
        </div>
    );
                    ...
                    <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
                        <AnimatePresence mode="wait">
                            ...
                            >
                                ...
                                {activeTab === 'personnel' && <PersonnelInviteTab />}
                            </motion.div>
                        </AnimatePresence>
                    </div>
